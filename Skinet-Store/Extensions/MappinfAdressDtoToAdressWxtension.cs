using Skinet_Store.Core.Entities;
using Skinet_Store.DTOs;

namespace Skinet_Store.Extensions
{
	public static class MappingAddressDtoToAddressExtension
	{

		public static Address MapAddressDtoToAddress(this AddressDto addressDto)
		{
			if (addressDto == null)
			{
				throw new ArgumentNullException(nameof(addressDto));
			}

			return new Address
			{
				Line1 = addressDto.Line1,
				Line2 = addressDto.Line2,
				City = addressDto.City,
				State = addressDto.State,
				PostalCode = addressDto.PostalCode
			};
		}


		public static AddressDto MapAddressToAddressDto(this Address address)
		{
			return new AddressDto
			{
				Line1 = address.Line1,
				Line2 = address.Line2,
				City = address.City,
				State = address.State,
				PostalCode = address.PostalCode
			};
		}

		public static void UpdateFromDto (this Address address, AddressDto addressDto)
		{
			address.Line1 = addressDto.Line1;
			address.Line2 = addressDto.Line2;
			address.City = addressDto.City;
			address.State = addressDto.State;
			address.PostalCode = addressDto.PostalCode;
		}
	}
}
